...
router.post('/edit', async (ctx, next) => {
    let data = ctx.request.body
    let body = await goodsDb.edit(data)
    ctx.body = body
    await next()
})
...
router.post('/getById', async (ctx, next) => {
    ...
    let res = await goodsDb.findById(id)
...
router.post('/getByIds', async (ctx, next) => {
    ...
    let data = await goodsDb.findIds(projectId)
...
router.post('/getByList', async (ctx, next) => {
    ...
    let data = await goodsDb.findAll({ projectId, name })
